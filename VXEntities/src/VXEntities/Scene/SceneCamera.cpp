...
namespace Vertex {
	...
	glm::vec2 SceneCamera::ScreenToWorldPoint(glm::vec2 screenPoint)
	{
		...
		glm::vec2 ndc;
		ndc.x = (2.0f * screenPoint.x) / m_ViewportWidth - 1.0f;
		ndc.y = 1.0f - (2.0f * screenPoint.y) / m_ViewportHeight;

		// Step 2: Create a clip-space position (z = -1 for 2D screen space)
		...
		// Step 5: Return the x, y components of the world position
